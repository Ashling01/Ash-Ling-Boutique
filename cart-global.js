...
class ShoppingCart {
    ...
    extractProductInfo(element) {
        ...
        const codeElement = card.querySelector('.product-code');
        ...
        // Extraer código del producto o generar uno si no existe
        let productCode = '';
        if (codeElement && codeElement.textContent.trim()) {
            productCode = codeElement.textContent.replace('Código: ', '').trim();
        } else {
            // Generar código temporal basado en el nombre de la imagen
            const imageName = image.split('/').pop().replace(/\.(jpg|jpeg|png|gif)$/i, '');
            const category = this.detectCategory();
            const prefix = this.getCategoryPrefix(category);
            productCode = `${prefix}-${imageName}`;
        }
    ...
    // Obtener prefijo de código basado en la categoría
    getCategoryPrefix(category) {
        switch(category) {
            case 'Carteras': return 'CAR';
            case 'Perfumes': return 'PERF';
            case 'Ropa de Dama': return 'RD';
            case 'Ropa de Caballero': return 'RC';
            case 'Accesorios': return 'ACC';
            default: return 'PROD';
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Event listeners se manejan individualmente en cada página
